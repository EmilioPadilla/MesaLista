...
import { seedPredesignedLists } from './seeds/predesignedLists.seed';
...
async function main() {
  ...
  // Create a gift list for the couple
  ...
  Sol & Emilio`,
      ...
      eventLocation: 'Querétaro, Querétaro',
      eventVenue: 'Casona de los 5 Patios',
      planType: 'COMMISSION',
      isActive: true,
    },
  });
  ...
  await Promise.all([
    ...
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[0].id, categoryId: cocinaCategory.id, giftListId: giftList.id },
    }),
    // Licuadora -> Electrodomésticos
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[1].id, categoryId: electroCategory.id, giftListId: giftList.id },
    }),
    // Juego de Toallas -> Baño
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[2].id, categoryId: banoCategory.id, giftListId: giftList.id },
    }),
    // TV -> Electrodomésticos
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[3].id, categoryId: electroCategory.id, giftListId: giftList.id },
    }),
    // Cena en Islandia -> Viaje
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[4].id, categoryId: viajeCategory.id, giftListId: giftList.id },
    }),
    // Tour por Dolomitas -> Viaje
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[5].id, categoryId: viajeCategory.id, giftListId: giftList.id },
    }),

    // Nespresso -> Electrodomésticos + Cocina (example of multiple categories)
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[6].id, categoryId: electroCategory.id, giftListId: giftList.id },
    }),
    prisma.giftCategoryOnGift.create({
      data: { giftId: gifts[6].id, categoryId: cocinaCategory.id, giftListId: giftList.id },
    }),
  ]);
  ...
  console.log(`Created ${4} users, 1 gift list and ${gifts.length} gifts`);

  // Seed predesigned lists
  await seedPredesignedLists();
}
