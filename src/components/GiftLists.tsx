...
import type { Gift, WeddingList } from '../api/giftService';
...
const GiftLists: React.FC<GiftListsProps> = () => {
  ...
  const { data: weddingLists, isLoading: isLoadingWeddingLists, refetch } = useQuery<WeddingList[]>({
  ...
  const filteredWeddingLists = weddingLists
    ? weddingLists.filter((list: WeddingList) => 
        list.coupleName.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : [];
    ...
    <div className="m-6">
      <div className="p-6 bg-white rounded-lg shadow">
        <div className="mb-6">
          ...
          <Paragraph>
            Explora las listas de regalos de las parejas y haz un regalo especial para su boda.
          </Paragraph>
        </div>
              ...
              <Card 
                ...
                title={
                  <div className="flex items-center">
                    ...
                    />
                    <div>
                      <Title level={4} className="mb-0">{weddingList.coupleName}</Title>
                      <Text type="secondary">Fecha de boda: {new Date(weddingList.weddingDate).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}</Text>
                    </div>
                  </div>
                }
              >
                <Row gutter={[16, 16]}>
                  {weddingList.gifts.map((gift: Gift) => (
                    <Col xs={24} sm={12} md={8} key={gift.id}>
                      <Card 
                        hoverable 
                        className={`h-full flex flex-col ${gift.isPurchased ? 'opacity-60' : ''}`}
                      >
                        <div className="flex-grow">
                          <div className="flex justify-between items-start mb-2">
                            <Title level={5} className="mb-1">{gift.title}</Title>
                            <Text strong className="text-lg">${gift.price.toFixed(2)}</Text>
                          </div>
        ...
        {selectedGift && (
          <div>
            <div className="mb-4">
              ...
              <Text strong className="text-lg">${selectedGift.price.toFixed(2)}</Text>
            </div>
