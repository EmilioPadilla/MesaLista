import {
  ...
  RSVPSection,
  FooterSection,
} from '../sections/InvitationFloralRomanceSections';
...
export function FloralRomanceTemplate({ data, formatDate, sections, palette = defaultPalette }: TemplateProps) {
      ...
      {enabledSections.length > 0 ? (
        ...
        <div className="relative bg-gradient-to-br from-rose-50 via-white to-pink-50 p-12 sm:p-16 min-h-[600px] flex flex-col items-center justify-center">
          <div className="space-y-8 max-w-xl text-center relative z-10">
            ...
            {data.date && <p className="text-lg text-foreground font-light capitalize">{formatDate(data.date)}</p>}
