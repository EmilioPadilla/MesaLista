...
describe('SettingsSidebar', () => {
  it('renders every group label and item label', () => {
    render(<SettingsSidebar activeSection="perfil" onSectionChange={vi.fn()} />);
    GROUPS.forEach((g) => {
  ...
  it('calls onSectionChange with item id when an item is clicked', () => {
    ...
    render(<SettingsSidebar activeSection="perfil" onSectionChange={onChange} />);

    fireEvent.click(screen.getByText('Contraseña'));
    expect(onChange).toHaveBeenCalledWith('contrasena');

    fireEvent.click(screen.getByText('Comisiones de pago'));
    expect(onChange).toHaveBeenCalledWith('comisiones');
  });
  ...
  it('applies red styling to the active danger item', () => {
    render(<SettingsSidebar activeSection="eliminar-cuenta" onSectionChange={vi.fn()} />);
